      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {filteredRequisitions.length === 0 ? (
          ...
          <div className="bg-white rounded-lg border border-[hsl(var(--canalco-neutral-200))] overflow-hidden">
            ...
            {filteredRequisitions.filter(r => ['en_orden_compra', 'pendiente_recepcion'].includes(r.status?.code || '')).length > 0 && (
              <div className={filteredRequisitions.filter(r => r.status?.code === 'cotizada').length > 0 ? 'border-t-4 border-[hsl(var(--canalco-neutral-200))]' : ''}>
                <div className="bg-green-50 border-b border-green-200 px-4 py-2">
                  <p className="text-sm font-semibold text-green-800 flex items-center gap-2">
                    ...
                    OC YA GENERADAS ({filteredRequisitions.filter(r => ['en_orden_compra', 'pendiente_recepcion'].includes(r.status?.code || '')).length})
                  </p>
                </div>
                <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredRequisitions.filter(r => ['en_orden_compra', 'pendiente_recepcion'].includes(r.status?.code || '')).map((req) => {
                  // Determinar última actualización según estado
                  const getLastAction = () => {
                    switch (req.status?.code) {
                      case 'cotizada':
